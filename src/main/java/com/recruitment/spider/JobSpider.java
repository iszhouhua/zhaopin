package com.recruitment.spider;
...
public class JobSpider {
	public static List<Job> getJobs(int page, String cityname, String name) throws Exception {
		...
		try {
			...
			Document doc = Jsoup.parse(new URL(joburl), 5000);
			Elements jobtable=doc.select("td.zwmc");//class等于zwmc的td标签
			int k=0;
			for(Element alist:jobtable){
				Element a=alist.select("a").first();
				k++;
